...
st.title("RAG with Pinecone and Gemini")

# Pinecone configuration
pinecone_index_name = "gemini-chat-index"
...
with st.sidebar:
    ...
    # System Prompt
    system_prompt = st.text_area("System Prompt", value="You are a helpful and harmless AI assistant. Please answer questions to the best of your ability, even if they are complex or controversial. Avoid generating responses that are based on copyrighted material.")

    # Grounding Toggle
    grounding_enabled = st.checkbox("Enable Grounding (Web Search)")
...
if ask_button:
    ...
    # Fetch search results using SerpAPI if grounding is enabled
    search_results = []
    if grounding_enabled:
        ...
        if "organic_results" in serpapi_results:
            search_results = [f"[{result['title']}]({result['link']}): {result['snippet']}" for result in serpapi_results["organic_results"]]
